import React from "react";
import { Image, View } from "react-native";
import * as Animatable from "react-native-animatable";
import AppStyle from "./styleApp/styleApp";

const App: React.FC = () => (
  <View style={AppStyle.AppCont}>
    <Animatable.View
      style={[AppStyle.fC, AppStyle.C]}
      animation="pulse"
      duration={250}
      iterationCount="infinite"
      iterationDelay={200}
      useNativeDriver={true}
      easing="linear"
    >
      <Animatable.View style={[AppStyle.fC, AppStyle.C]}></Animatable.View>
      <Animatable.Image
        source={{
          uri:
            "data:image/jpeg;base64,"
        }}
        style={[AppStyle.fC]}
        useNativeDriver={true}
        animation="rotate"
        duration={500}
        iterationCount="infinite"
        easing="linear"
      />
      <View style={[AppStyle.fC, AppStyle.sC]}>
        <Image
          style={[AppStyle.fC, AppStyle.sC]}
          source={{
            uri: "https://media.giphy.com/media/26BRqkvsYzYVCicne/giphy.gif"
          }}
        />
      </View>
    </Animatable.View>
  </View>
);

export default App;
